...
class MovieLen(object):
    def __init__(self, use_poster):
        ...
        # 声明每个数据文件的路径
        usr_info_path = "D:/data/paddle/ml-1m/ml-1m/users.dat"
        if use_poster:
        ...
        self.poster_path = "D:/data/paddle/ml-1m/ml-1m/posters/"
        # 得到电影数据
...
class Model(paddle.nn.Layer):
    def __init__(self, use_poster, use_mov_title, use_mov_cat, use_age_job, fc_sizes):
        ...
        self.mov_cat_emb = Embedding(num_embeddings=CATEGORY_DICT_SIZE, embedding_dim=category_embedding_dim,
                                     sparse=False)
        self.mov_cat_fc = Linear(in_features=category_embedding_dim, out_features=32)

        # 对电影名称做映射
        MOV_TITLE_DICT_SIZE = len(Dataset.movie_title) + 1
        self.mov_title_emb = Embedding(num_embeddings=MOV_TITLE_DICT_SIZE, embedding_dim=title_embedding_dim,
                                       sparse=False)
...
def get_usr_mov_features(model, params_file_path, poster_path):
    ...
    # 保存特征到本地
    pickle.dump(usr_pkl, open('model/usr_feat.pkl', 'wb'))
    pickle.dump(mov_pkl, open('model/mov_feat.pkl', 'wb'))
