    ...
    private static Scope getRootScope() {
        if (root == null) {
            synchronized (Scope.class) {
                ...
                root.create("true").makeConstant(1);
                root.create("false").makeConstant(0);
                root.create("NaN").makeConstant(Double.NaN);
                ...
                root.create("today").makeConstant(c.getTimeInMillis());
                root.create("current_date").makeConstant(c.get(Calendar.DATE));
                root.create("current_month").makeConstant(c.get(Calendar.MONTH) + 1);
            }
        }
