...
import com.lvmoney.frame.expression.tokenizer.Token.TokenType;
...
public class Tokenizer extends Lookahead<Token> {
    ...
    public static final char varIndxSplit = '#';

    public static final char indxRange = '~';
     ...
     */
    private char[] brackets = {'(', '[', CHAR_BRACE_LEFT, CHAR_BRACE_RIGHT, ']', ')'};
    /**
     ...
     * escape characters within this string. CHAR_ZERO_TRANS means no escaping.
     */
    ...
    public Tokenizer(Reader input) {
        ...
        addStringDelimiter(CHAR_DOUBLE_QUOTATION_MARK, CHAR_SLASH);
        addStringDelimiter(CHAR_SINGE_QUOTATION_MARK, CHAR_ZERO_TRANS);
    }
     ...
     * <p>
     * By default this is either indicated by a digit or by CHAR_REDUCE followed by a digit or a CHAR_DECIMAL_POINT followed by a digit.
     *
    ...
    protected boolean isAtStartOfNumber() {
        return input.current().isDigit()
                || input.current().is(CHAR_REDUCE) && input.next().isDigit()
                || input.current().is(CHAR_REDUCE) && input.next().is(CHAR_DECIMAL_POINT) && input.next(2).isDigit()
                || input.current().is(CHAR_DECIMAL_POINT) && input.next().isDigit();
    }
     ...
     * single CHAR_OPERATION_SYMBOL_OR is also treated as bracket.
     *
    ...
    protected boolean isAtBracket(boolean inSymbol) {
        return input.current().is(brackets) || !inSymbol
                ...
                && input.current().is(CHAR_OPERATION_SYMBOL_OR)
                && !input.next().is(CHAR_OPERATION_SYMBOL_OR);
    }

    protected boolean isAtBrace() {
        return input.current().is(CHAR_BRACE_LEFT);
    }
    ...
    protected Token fetchString() {
        ...
        while (!input.current().isNewLine() && !input.current().is(separator) && !input.current().isEndOfInput()) {
            if (escapeChar != CHAR_ZERO_TRANS && input.current().is(escapeChar)) {
    ...
    protected boolean handleStringEscape(char separator, char escapeChar, Token stringToken) {
        if (input.current().is(separator)) {
            ...
            stringToken.addToSource(input.consume());
            return true;
        } else if (input.current().is(CHAR_N)) {
            stringToken.silentAddToContent(CHAR_N_TRANS);
            stringToken.addToSource(input.consume());
            return true;
        } else if (input.current().is(CHAR_R)) {
            stringToken.silentAddToContent(CHAR_R_TRANS);
            stringToken.addToSource(input.consume());
            return true;
        } else {
            return false;
     ...
     * <p>
     * By default, letters, digits and CHAR_CONNECTOR_UNDERLINE are valid identifier parts.
     *
    ...
    protected boolean isIdentifierChar(Char current) {
        return current.isDigit() || current.isLetter() || current.is(CHAR_CONNECTOR_UNDERLINE)
                || current.is(varIndxSplit) || current.is(indxRange)
                || current.is(decimalSeparator);
    }
    ...
    protected Token fetchSymbol() {
        ...
        result.addToTrigger(input.consume());
        if (result.isSymbol(OPERATION_SYMBOL_MULTIPLICATION) && input.current().is(CHAR_OPERATION_SYMBOL_MULTIPLICATION)
                || result.isSymbol(OPERATION_SYMBOL_AND) && input.current().is(CHAR_OPERATION_SYMBOL_AND)
                || result.isSymbol(OPERATION_SYMBOL_OR) && input.current().is(CHAR_OPERATION_SYMBOL_OR)
                || result.isSymbol() && input.current().is(CHAR_EQUAL)) {

        }
        if (result.isSymbol(OPERATION_SYMBOL_MULTIPLICATION) && input.current().is(CHAR_OPERATION_SYMBOL_MULTIPLICATION)) {
            result.addToTrigger(input.consume());
        } else if (result.isSymbol(OPERATION_SYMBOL_AND) && input.current().is(CHAR_OPERATION_SYMBOL_AND)) {
            result.addToTrigger(input.consume());
        } else if (result.isSymbol(OPERATION_SYMBOL_OR) && input.current().is(CHAR_OPERATION_SYMBOL_OR)) {
            result.addToTrigger(input.consume());
        } else if (result.isSymbol(OPERATION_SYMBOL_LESS) && input.current().is(CHAR_EQUAL)) {
            result.addToTrigger(input.consume());
        } else if (result.isSymbol(OPERATION_SYMBOL_GREATER) && input.current().is(CHAR_EQUAL)) {
            result.addToTrigger(input.consume());
        } else if (result.isSymbol(OPERATION_SYMBOL_NOT) && input.current().is(CHAR_EQUAL)) {
            result.addToTrigger(input.consume());
        } else if (result.isSymbol(OPERATION_SYMBOL_EQUAL) && input.current().is(CHAR_EQUAL)) {
            result.addToTrigger(input.consume());
        }
        return result;
    }

    protected Token fetchUnit() {
        Token result = Token.create(Token.TokenType.UNIT, input.current());
        result.addToTrigger(input.consume());

        while (!input.current().isNewLine() && !input.current().isEndOfInput()) {
            result.addToContent(input.consume());
            if (input.current().is(CHAR_BRACE_RIGHT)) {
                result.addToContent(input.consume());
                break;
            }
        }
    ...
    protected Token fetchNumber() {
       ...
       /* while (input.current().isDigit() || input.current().is(decimalSeparator) || (input.current()
        ...
        while (input.current().isDigit()
                ...
                && (input.next().isDigit() || input.next().is(CHAR_PLUS) || input.next().is(CHAR_REDUCE)))) {
            ...
            //add by liutq 科学记数法   2020.2.18
                ...
                if (result.is(TokenType.SCIENTIFIC_DECIMAL)) {
                    ...
                    input.consume();
                    if (input.current().is(CHAR_PLUS) || input.current().is(CHAR_REDUCE)) {
                        result.addToContent(input.consume());
                    }
                }
            }
            // end add by liutq 科学记数法   2020.2.18
            else {
                result.addToContent(input.consume());
            }
        }
    ...
    public void addUnescapedStringDelimiter(char stringDelimiter) {
        stringDelimiters.put(stringDelimiter, CHAR_ZERO_TRANS);
    }

    /**
     * Returns the decimal separator used in decimal numbers
     * <p>
     * The default separator used is CHAR_DECIMAL_POINT
     *
     ...
     * <p>
     * The default separator used is CHAR_DECIMAL_POINT. When adapting this for language dependent inputs (e.g. using ',' as
     ...
     * <p>
     * This character will be accepted in numbers, but ignored (not added to the content). The default value is CHAR_CONNECTOR_UNDERLINE.
     *
     ...
     * <p>
     * The default value is '/'CHAR_OPERATION_SYMBOL_MULTIPLICATION
     *
